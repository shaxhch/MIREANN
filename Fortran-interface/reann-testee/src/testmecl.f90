program mypes 
    ...
    integer,parameter :: luckatom=2,RCnum=2
    integer::RCindex(2,RCnum)
    real(kind=8)::RCeff(RCnum),rc
    RCindex=reshape([1,3,2,3],[2,RCnum])
    RCeff=[-1,1]
    ...
    allocate(coor_qm(chnumatoms*3),coor_mm(chmaxnum_mm*3),cha_mm(chmaxnum_mm),force_qm(chnumatoms*3),distance(chmaxnum_mm), &
             ...
             pot_ee(chmaxnum_mm),pot_me(chmaxnum_mm),pot_eenn(chmaxnum_mm),pot_menn(chmaxnum_mm))
    npoint=1
    ...
    do
      coor_mm = 0d0;cha_mm = 0d0;cha_ee = 0d0;cha_me = 0d0;cha_eenn = 0d0;cha_menn = 0d0
      pot_eenn = 0d0;pot_menn = 0d0;pot_ee = 0d0;pot_me = 0d0;rc = 0d0
      ...
      num_mm = 0
      do
        read(181,'(F12.6,2F13.6,F10.4,F12.6)',iostat=istat) coor_mm(num_mm*3+1:num_mm*3+3),    &
      ...
      do i = 0,num_mm-1
        read(183,'(F12.6,2F13.6,F5.1,F12.6)',iostat=istat) coor_mm(i*3+1:i*3+3), cha_mm(i+1), pot_me(i+1)
      end do
      ...
      do j = 1,num_mm
        write(182,'(2F10.4,4F16.8)') rc,distance(j),pot_me(j),pot_menn(j),pot_ee(j),pot_eenn(j)
      end do
      npoint=npoint+1
    end do
