program mypes 
    ...
    do
      ...
      do j = 1,num_mm
        write(182,'(2F10.4,4F16.8)') rc,distance(j),pot_me(j),pot_menn(j),pot_mePoint(j),pot_ee(j),pot_eenn(j),pot_mePoint(j)
      end do
      npoint=npoint+1
    end do
