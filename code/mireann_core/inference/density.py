...
class GetDensity(torch.nn.Module):
    def __init__(self,rs,inta,dceff,cutoff,nipsin,norbit,ocmod_list):
        ...
        self.dceff=nn.parameter.Parameter(dceff)
        ...
        self.hyper=nn.parameter.Parameter(torch.nn.init.orthogonal_(torch.rand(self.rs.shape[1],norbit)).\
    ...
    def calculate_bg_field(self, cart, bgcart, bgcha):
        delta = cart.unsqueeze(1)-bgcart.unsqueeze(0)
        distance = torch.norm(delta,dim=-1,keepdim=True)+1e-7
        bg_field = torch.sum(bgcha.unsqueeze(0).unsqueeze(-1)*delta/distance.pow(3),dim=1)
        return bg_field.view(-1,  3)
    ...
    def forward(self,cart,bgcart,bgcha,neigh_list,shifts,species):
        """
        ...
        density,worbital=self.obtain_orb_coeff(0,numatom,orbital,bg_orbital,neigh_list,orb_coeff,hyper)
        for ioc_loop, (_, m) in enumerate(self.ocmod.items()):
            ...
            density,worbital = self.obtain_orb_coeff(ioc_loop+1,numatom,orbital,bg_orbital,neigh_list,orb_coeff,hyper)
        return density

    def obtain_orb_coeff(self,iteration:int,numatom:int,orbital,bg_orbital,neigh_list,orb_coeff,hyper):
        expandpara=orb_coeff.index_select(0,neigh_list[1])
        worbital=torch.einsum("ijk,ik ->ijk", orbital,expandpara)
        worbital=torch.einsum("ijk,ik->ijk", orbital,expandpara).contiguous()
        bg_orbital=torch.index_add(bg_orbital,0,neigh_list[0],worbital)
        hyper_worbital=torch.einsum("ijk,jkm -> ijm",bg_orbital,hyper).contiguous()
        density=torch.sum(torch.square(hyper_worbital),dim=1)
        return density,bg_orbital
