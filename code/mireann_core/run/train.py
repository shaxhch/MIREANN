...
dataloader_val=DataLoader(com_coor_val,bgcor_val,bgcha_val,bgcha_real_val,abpropset_val,numatoms_val,\
...
if start_table <= 5:
    ...
    Prop_class=Property(getdensity,totcharge,nnmodlist,gausswidth,hardness,polarizability,slatR,isFreezeDensity,ckptPath).to(device).to(torch_dtype)
...
if dist.get_rank()  == 0:
    ...
    header = f"Parameter Summary {timestamp}".center(110, '=')
    print(header)
    total_params = 0 
    for name, param in Prop_class.named_parameters(): 
        param_count = param.numel() 
        output_line = f"{name.ljust(70)}  | Total：{str(param_count).rjust(10)} | Shape：{tuple(param.shape)}" 
        print(output_line)
        total_params += param_count 
    summary = f"Parameters in total：{total_params} | Trainable groups：{len(list(Prop_class.parameters()))}" 
    print(summary)
    fout.flush() 
#==========================================================
Optimize(fout,prop_ceff,nprop,train_nele,val_nele,init_f,final_f,decay_factor,start_lr,end_lr,print_epoch,Epoch,\
data_train,data_val,Prop_class,loss_fn,optim,scheduler,ema,restart,PES_Normal,device,isFreezeDensity,PES_Lammps=PES_Lammps)
if dist.get_rank()==0:
